/*
  BADR AI V2.3 — Pricing Intelligence + Full-Site Developer Brain
  Cloudflare Worker
  Requires existing Workers AI binding named: AI

  IMPORTANT:
  1) Upload assets/badr-ai/badr-ai-knowledge-v2-3.json to the BADR website/GitHub Pages.
...
const KNOWLEDGE_URLS = [
  ...
  'https://badratelier.com/assets/badr-ai/badr-ai-knowledge-v2-3.json',
  'https://hhadhood.github.io/BADR-Atelier-/assets/badr-ai/badr-ai-knowledge-v2-3.json'
];
...
const FALLBACK = {
  ...
  team: { departments: [], stats: { specialists:'40+', studioStreams:'12' } },
...
function detectMode(text,path=''){
  ...
  if(containsAny(q,['فريق','team','مهندس','architect','مين','who is','staff','founder','مؤسس','هاني','yousra','يسرى','يسرا','bim team','finance team'])) return 'team';
...
function fastPricingAnswer(q,kb,ar){
  ...
  return ar
    ? `${rateLine}\n\nالنطاق المتكامل هو أتعاب أعمال التصميم والهندسة: ${disciplines.join(' + ')}، وليس تكلفة إنشاء المشروع. للحسبة أرسل لي المساحة التصميمية/المبنية الداخلة في نطاق العمل ونوع المشروع. الأسعار هنا بالريال السعودي وما يقاربها بالدولار فقط — لا نعرض أسعارًا بالجنيه المصري.`
    : `${rateLine}\n\nThe integrated scope is a professional design/engineering fee covering ${disciplines.join(' + ')}, not project construction cost. To calculate, send the confirmed design/built-up area and project type. BADR AI shows pricing only in SAR and approximate USD — never EGP.`;
}
...
function findMember(q,kb){
  ...
  const aliases=[['يسرا','yousra'],['يسرى','yousra'],['هاني','hani']];
...
export default {
  async fetch(request,env){
    ...
    if(request.method==='GET') return jsonResponse({ok:true,assistant:'BADR AI V2.3',status:'online',brain:'Pricing Intelligence + Full-Site Developer Brain',knowledgeVersion:kb.version||'unknown',engine:'Cloudflare Workers AI'},200,request);
    ...
    if(mode==='pricing'){
      ...
      if(reply) return jsonResponse({reply,actions:actionsFor(mode,lead,lang,kb,[]),assistant:'BADR AI V2.3',mode,modeLabel:ar?label.ar:label.en,confidence:'high',leadStage:lead,knowledgeVersion:kb.version,fastFact:true},200,request);
    }
    if(mode==='contact'){
...
- BADR approved public fee reference: integrated professional design/engineering scope is SAR 60–100/m² of CONFIRMED DESIGN AREA; design-only is 40% of the integrated fee = SAR 24–40/m². Show pricing ONLY in SAR and approximate USD (reference 1 USD ≈ 3.75 SAR). NEVER show EGP. Never automatically price plot/land area. The integrated fee covers Architecture + Structural + Electrical + Mechanical + Plumbing/Electromechanical coordination and is NOT construction cost. Final proposal depends on project type, complexity, deliverables and agreed scope.
...
Allowed price currencies: SAR and USD only. Never EGP.
...
- For ANY factual question about BADR, search and use the retrieved site evidence.
...
- Project / land / investment question: think like a developer and use compact structure.
...
`;
    ...
    try{
      ...
      if(!reply){console.log('BADR AI V2.3 empty response',JSON.stringify(result));return jsonResponse({error:'Empty AI response'},502,request);}
      return jsonResponse({
        reply,
        actions:actionsFor(mode,lead,lang,kb,selected),
        assistant:'BADR AI V2.3',
      ...
      console.error('BADR AI V2.3 Worker error',error);
