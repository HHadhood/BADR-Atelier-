/*
  BADR AI V2.4 — Executive Knowledge + Full-Site Real Estate Intelligence
  Cloudflare Worker
  Requires existing Workers AI binding named: AI

  IMPORTANT:
  1) Upload assets/badr-ai/badr-ai-knowledge-v2-4.json to the BADR website/GitHub Pages.
...
const KNOWLEDGE_URLS = [
  'https://www.badratelier.com/assets/badr-ai/badr-ai-knowledge-v2-4.json',
  'https://badratelier.com/assets/badr-ai/badr-ai-knowledge-v2-4.json',
  'https://hhadhood.github.io/BADR-Atelier-/assets/badr-ai/badr-ai-knowledge-v2-4.json'
];
...
const FALLBACK = {
  ...
  pricing: { currencyPolicy:{allowed:['SAR','USD'],usdSarReference:3.75}, basis:{nameEn:'Confirmed design area',nameAr:'المساحة التصميمية المؤكدة'}, integratedProfessionalScope:{sarPerM2:{min:60,max:100},usdPerM2:{min:16,max:26.67},includedDisciplinesEn:['Architecture','Structural Engineering','Electrical','Mechanical','Plumbing / Electromechanical coordination'],includedDisciplinesAr:['معماري','إنشائي','كهرباء','ميكانيكا','صحي / تنسيق كهروميكانيكال']}, designShare:{percentOfIntegratedFee:40,sarPerM2Derived:{min:24,max:40},usdPerM2Derived:{min:6.4,max:10.67}} }
};
...
function fastPricingAnswer(q,kb,ar){
  ...
  const fullSar=full.sarPerM2||{min:60,max:100};
  const desSar=des.sarPerM2Derived||{min:24,max:40};
  const fullUsd=full.usdPerM2||{min:16,max:26.67};
  const desUsd=des.usdPerM2Derived||{min:6.4,max:10.67};
  const disciplines=ar?(full.includedDisciplinesAr||[]): (full.includedDisciplinesEn||[]);
  const rateLine = ar
    ? `• الأعمال المهنية المتكاملة: ${fullSar.min}–${fullSar.max} ريال/م² (حوالي ${fullUsd.min.toFixed(2)}–${fullUsd.max.toFixed(2)} دولار/م²).\n• التصميم فقط: 40% من أتعاب النطاق المتكامل = ${desSar.min}–${desSar.max} ريال/م² (حوالي ${desUsd.min.toFixed(2)}–${desUsd.max.toFixed(2)} دولار/م²).`
    : `• Integrated professional scope: SAR ${fullSar.min}–${fullSar.max}/m² (about USD ${fullUsd.min.toFixed(2)}–${fullUsd.max.toFixed(2)}/m²).\n• Design-only: 40% of the integrated fee = SAR ${desSar.min}–${desSar.max}/m² (about USD ${desUsd.min.toFixed(2)}–${desUsd.max.toFixed(2)}/m²).`;
  if(plotOnly && area){
    return ar
      ? `مساحة ${area.toLocaleString('ar-SA')} م² تبدو في سؤالك مساحة أرض. تسعير BADR لا يضرب مساحة الأرض تلقائيًا؛ نحتاج المساحة التصميمية/المبنية الداخلة في نطاق العمل أولًا.\n\n${rateLine}\n\nالنطاق المتكامل يشمل: ${disciplines.join(' + ')}. الأسعار عبر BADR AI بالريال السعودي أو الدولار فقط، وهي استرشادية وليست عرض سعر نهائي.`
      : `The ${area.toLocaleString('en-US')} m² in your question appears to be plot/land area. BADR does not automatically multiply plot area for fees; we first need the confirmed design/built-up area in scope.\n\n${rateLine}\n\nIntegrated scope includes: ${disciplines.join(' + ')}. The fee reference is shown in SAR first, followed by an approximate USD equivalent. Figures are indicative, not a final quotation.`;
  }
  if(area){
    const fmin=area*fullSar.min, fmax=area*fullSar.max, dmin=area*desSar.min, dmax=area*desSar.max;
    const fumin=area*fullUsd.min, fumax=area*fullUsd.max, dumin=area*desUsd.min, dumax=area*desUsd.max;
    if(scope==='integrated') return ar
      ? `للمساحة التصميمية المؤكدة ${area.toLocaleString('ar-SA')} م²:\n• التقدير الاسترشادي للنطاق المتكامل: ${money(fmin,'SAR',ar)} – ${money(fmax,'SAR',ar)}\n• ما يقاربه بالدولار: ${money(fumin,'USD',ar)} – ${money(fumax,'USD',ar)}\n\nيشمل النطاق المتكامل: ${disciplines.join(' + ')}. اختيار المعدل داخل 60–100 ريال/م² يعتمد على نوع المشروع وتعقيده والمخرجات المطلوبة. هذا تقدير أولي وليس عرض سعر ملزمًا.`
      : `For ${area.toLocaleString('en-US')} m² of confirmed design area:\n• Indicative integrated scope: ${money(fmin,'SAR',ar)} – ${money(fmax,'SAR',ar)}\n• Approx. USD: ${money(fumin,'USD',ar)} – ${money(fumax,'USD',ar)}\n\nIntegrated scope includes ${disciplines.join(' + ')}. The point within SAR 60–100/m² depends on project type, complexity and deliverables. This is preliminary, not a binding quotation.`;
    if(scope==='design') return ar
      ? `للتصميم فقط على مساحة تصميمية مؤكدة ${area.toLocaleString('ar-SA')} م²:\n• 40% من أتعاب النطاق المتكامل = ${money(dmin,'SAR',ar)} – ${money(dmax,'SAR',ar)}\n• ما يقاربه بالدولار: ${money(dumin,'USD',ar)} – ${money(dumax,'USD',ar)}\n\nهذا يعادل استرشاديًا ${desSar.min}–${desSar.max} ريال/م². العرض النهائي يتحدد بعد معرفة نوع المشروع ومستوى التعقيد والمخرجات.`
      : `For design-only on ${area.toLocaleString('en-US')} m² of confirmed design area:\n• 40% of the integrated fee = ${money(dmin,'SAR',ar)} – ${money(dmax,'SAR',ar)}\n• Approx. USD: ${money(dumin,'USD',ar)} – ${money(dumax,'USD',ar)}\n\nThat is an indicative SAR ${desSar.min}–${desSar.max}/m². Final fees are confirmed after project type, complexity and deliverables are defined.`;
    return ar
      ? `للمساحة التصميمية المؤكدة ${area.toLocaleString('ar-SA')} م²:\n• متكامل: ${money(fmin,'SAR',ar)} – ${money(fmax,'SAR',ar)} (حوالي ${money(fumin,'USD',ar)} – ${money(fumax,'USD',ar)}).\n• تصميم فقط: ${money(dmin,'SAR',ar)} – ${money(dmax,'SAR',ar)} (حوالي ${money(dumin,'USD',ar)} – ${money(dumax,'USD',ar)}).\n\nالمتكامل يشمل ${disciplines.join(' + ')}. إذا أخبرتني نوع المشروع ودرجة التعقيد أساعدك في قراءة أين يقع منطقيًا داخل النطاق، مع بقاء السعر النهائي مع فريق BADR.`
      : `For ${area.toLocaleString('en-US')} m² of confirmed design area:\n• Integrated: ${money(fmin,'SAR',ar)} – ${money(fmax,'SAR',ar)} (about ${money(fumin,'USD',ar)} – ${money(fumax,'USD',ar)}).\n• Design-only: ${money(dmin,'SAR',ar)} – ${money(dmax,'SAR',ar)} (about ${money(dumin,'USD',ar)} – ${money(dumax,'USD',ar)}).\n\nIntegrated scope includes ${disciplines.join(' + ')}. Share project type and complexity and I can help position it within the range; BADR confirms the final fee formally.`;
  }
  return ar
    ? `${rateLine}\n\nالنطاق المتكامل هو أتعاب أعمال التصميم والهندسة: ${disciplines.join(' + ')}، وليس تكلفة إنشاء المشروع. للحسبة أرسل لي المساحة التصميمية/المبنية الداخلة في نطاق العمل ونوع المشروع. الأسعار هنا بالريال السعودي وما يقاربها بالدولار.`
    : `${rateLine}\n\nThe integrated scope is a professional design/engineering fee covering ${disciplines.join(' + ')}, not project construction cost. To calculate, send the confirmed design/built-up area and project type. BADR AI shows pricing in SAR with an approximate USD equivalent.`;
}
...
export default {
  async fetch(request,env){
    ...
    if(request.method==='GET') return jsonResponse({ok:true,assistant:'BADR AI V2.4',status:'online',brain:'Executive Knowledge + Full-Site Real Estate Intelligence',knowledgeVersion:kb.version||'unknown',engine:'Cloudflare Workers AI'},200,request);
    ...
    if(mode==='pricing'){
      ...
      if(reply) return jsonResponse({reply,actions:actionsFor(mode,lead,lang,kb,[]),assistant:'BADR AI V2.4',mode,modeLabel:ar?label.ar:label.en,confidence:'high',leadStage:lead,knowledgeVersion:kb.version,fastFact:true,sources:[{title:'BADR Site',url:mode==='contact'?'contact.html':mode==='team'?'team.html':'services.html',section:mode==='pricing'?'Pricing / Services':'Verified Facts'}]},200,request);
    }
    if(mode==='contact'){
    ...
    if(mode==='clients'){
      ...
      if(reply) return jsonResponse({reply,actions:actionsFor(mode,lead,lang,kb,[]),assistant:'BADR AI V2.4',mode,modeLabel:ar?label.ar:label.en,confidence:'high',leadStage:lead,knowledgeVersion:kb.version,fastFact:true,sources:[{title:'Projects / Clients',url:'projects.html',section:'Diplomatic + Published Portfolio'}]},200,request);
    }
    if(mode==='portfolio_stats'){
      const reply=fastPortfolioStatsAnswer(last,kb,ar);
      if(reply) return jsonResponse({reply,actions:actionsFor(mode,lead,lang,kb,[]),assistant:'BADR AI V2.4',mode,modeLabel:ar?label.ar:label.en,confidence:'high',leadStage:lead,knowledgeVersion:kb.version,fastFact:true,sources:[{title:'Projects',url:'projects.html',section:'Published Portfolio'}]},200,request);
    }
    if(mode==='company'){
      const reply=fastCompanyAnswer(last,kb,ar);
      if(reply) return jsonResponse({reply,actions:actionsFor(mode,lead,lang,kb,[]),assistant:'BADR AI V2.4',mode,modeLabel:ar?label.ar:label.en,confidence:'high',leadStage:lead,knowledgeVersion:kb.version,fastFact:true,sources:[{title:'About BADR',url:'about.html',section:'Studio Facts'}]},200,request);
    }
...
Integrated professional scope: SAR 60–100/m² of confirmed design area.
Design-only: 40% of integrated fee = SAR 24–40/m².
...
`;
    ...
    try{
      ...
      if(!reply){console.log('BADR AI V2.4 empty response',JSON.stringify(result));return jsonResponse({error:'Empty AI response'},502,request);}
      return jsonResponse({
        reply,
        actions:actionsFor(mode,lead,lang,kb,selected),
        assistant:'BADR AI V2.4',
      ...
      console.error('BADR AI V2.4 Worker error',error);
